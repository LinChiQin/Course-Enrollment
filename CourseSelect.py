...
def GetSelectList():
    ...
    req = requests.get(url, headers={
        'Cookie': 'JSESSIONID=69D54B17F91366BAFCEFB6D61B897E62; SERVERID=128; JSESSIONID=BA278209CBF5A0E998104B119FA8AADC',
...
def SelectCourse(course_list_url, course_check_url, data, inquiry_params):
    ...
    courses = []
    try:
        courses_dict = json.loads(req.text)
        print("获取课程成功!")
    except:
        print("登录失败！，执行二次登录")
    ...
    while jx0404id != '0':
        ...
        for course in courses:
            if jx0404id == course[4]:
                for i in range(3):
                    th = threading.Thread(target=DoSelect , args = (course_check_url, course))
                    th.start()
                break
